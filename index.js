...
app.get("/api/history/withdraw", requireLogin, async (req, res) => {
  const user = await User.findById(req.session.userId).select("historyWithdraw");
  res.json({
    success: true,
...
app.get("/api/user-saldo", requireLogin, async (req, res) => {
  const user = await User.findById(req.session.userId).select("saldo");
...
app.post("/withdraw/create", requireLogin, async (req, res) => {
  try {
    const user = await User.findById(req.session.userId);
    if (!user) {
      return res.status(401).json({
    ...
    const { metode, tujuan, nominal } = req.body;
    const amount = parseInt(nominal);

    // ===== VALIDASI =====
    if (!metode)
      return res.json({ success:false, message:"Metode belum dipilih" });

    if (!tujuan || tujuan.length < 6)
      return res.json({ success:false, message:"Nomor tujuan tidak valid" });

    if (!amount || amount < 10000)
      return res.json({ success:false, message:"Minimal withdraw Rp10.000" });
    ...
    const response = await cloudscraper.post(
      `${BASE_URL}/withdraw/create`,
      { body: payload, headers: cloudscraperHeaders }
    );
    ...
    if (!result.status) {
      return res.json({
        success:false,
        message: result.message || "Withdraw gagal"
      });
    }
    ...
    // ===== SIMPAN DATA =====
    if (!user.historyWithdraw) user.historyWithdraw = [];

    user.saldo -= amount;

    user.historyWithdraw.push({
      id: wd.id,
      reff_id: wd.reff_id,
      metode,
      tujuan,
      nominal: amount,
      fee: parseInt(wd.fee) || 0,
      status: wd.status,
      provider_id: wd.id,
      created_at: new Date()
    });
    ...
    // ===== AUTO CEK STATUS (MAX 2 MENIT) =====
    let attempts = 0;

    const intervalId = setInterval(async () => {
      try {
        attempts++;
        if (attempts > 40) return clearInterval(intervalId);

        const checkPayload = qs.stringify({
          api_key: ATLAN_API_KEY,
          id: wd.id
        });

        const checkRes = await cloudscraper.post(
          `${BASE_URL}/withdraw/status`,
          { body: checkPayload, headers: cloudscraperHeaders }
        );

        const statusData = JSON.parse(checkRes);
        const newStatus = statusData?.data?.status;
        if (!newStatus) return;

        const userNow = await User.findOne({
          _id: user._id,
          "historyWithdraw.id": wd.id
        });

        if (!userNow) return;

        const trx = userNow.historyWithdraw.find(x => x.id === wd.id);
        if (!trx) return;

        if (trx.status === newStatus) return;
        ...
        if (["success", "failed", "cancel"].includes(newStatus)) {
          clearInterval(intervalId);
        }

      } catch (err) {
        console.log("Cek status withdraw error:", err.message);
      }
    }, 3000);
