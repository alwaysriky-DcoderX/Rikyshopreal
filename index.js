...
app.post("/withdraw/create", requireLogin, async (req, res) => {
  try {
    ...
    if (amount < 10000) {
      return res.status(400).json({
        success: false,
        message: "Minimal withdraw Rp10.000"
      });
    }
    ...
    if (!result || !result.status || !result.data) {
      return res.status(502).json({
        success: false,
        message: result?.message || "Gagal membuat withdraw ke provider",
        error: result
      });
    }
    ...
    // =========================
    // AUTO CEK STATUS WITHDRAW
    // =========================
    const intervalId = setInterval(async () => {
      try {
        const checkResponse = await cloudscraper.post(
        ...
        const statusData = JSON.parse(checkResponse);
        ...
        if (statusData?.status && statusData?.data) {
          ...
          await User.updateOne(
            ...
            { $set: { "historyWithdraw.$.status": currentStatus } }
          );
          ...
          if (["success", "failed", "cancel"].includes(currentStatus)) {
            clearInterval(intervalId);
          }
        }

      } catch (err) {
        console.error("Cek status withdraw gagal:", err.message);
      }
    }, 1000);
    ...
    res.status(500).json({
      success: false,
      message: "Terjadi kesalahan internal saat withdraw"
    });
  }
});
